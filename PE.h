...
private:
    ...
    // HEADERS
    IMAGE_DOS_HEADER     PEFILE_DOS_HEADER;
    IMAGE_NT_HEADERS32   PEFILE_NT_HEADERS;
    ...
    void ParseNTHeaders(){
        fseek(Ppefile,PEFILE_DOS_HEADER_LFANEW,SEEK_SET);
        fread(&PEFILE_NT_HEADERS,sizeof(IMAGE_NT_HEADERS32),1,Ppefile);
    ...
    void ParseSectionHeaders(){
        PEFILE_SECTION_HEADERS=new IMAGE_SECTION_HEADER[PEFILE_NT_HEADERS_FILE_HEADER_NUMBER0F_SECTIONS];
        long offset=PEFILE_DOS_HEADER_LFANEW+sizeof(IMAGE_NT_HEADERS32);
        for (int i=0;i<PEFILE_NT_HEADERS_FILE_HEADER_NUMBER0F_SECTIONS;i++){
            fseek(Ppefile,offset,SEEK_SET);
            fread(PEFILE_SECTION_HEADERS+i,IMAGE_SIZEOF_SECTION_HEADER,1,Ppefile);
            offset+=IMAGE_SIZEOF_SECTION_HEADER;
    ...
    void PrintImportTableInfo(){
        ...
        for (int i=0;i<_import_directory_count;i++){
            ...
            while(true){
                ILT_ENTRY_32 tmpILT;
                fseek(Ppefile,ILTAddr,SEEK_SET);
                fread(&tmpILT,sizeof(ILT_ENTRY_32),1,Ppefile);
                if(tmpILT.FIELD_1.HINT_NAME_TABE==0){
                    break;
                }
                else if (tmpILT.FIELD_1.ORDINAL_NAME_FLAG==1){
                    int ordinal = tmpILT.FIELD_1.ORDINAL;
                    printf(" \t** 0x%X\n",ordinal);

                }
                else if ((tmpILT.FIELD_1.ORDINAL_NAME_FLAG)==0){
                    int sizeFuncName=0;
                    long addrFuncName= resolve(tmpILT.FIELD_1.HINT_NAME_TABE,locate(tmpILT.FIELD_1.HINT_NAME_TABE));
                    while(true){
                        char tmpChar;
                        fseek(Ppefile,addrFuncName+sizeFuncName,SEEK_SET);
                        fread(&tmpChar,sizeof(char),1,Ppefile);
